...
public:
	...
	explicit VisualizationPublisherGM(ros::NodeHandle n) : nh_(n), target_frame_("map")
	{
		...
		if (line[0] == '[')
		{
			...
			if (word != NULL)
			{
				...
				for (int it = 0; word[it] != '\0'; it++)
				{
					for (int jt = 0; illegalword[jt] != '\0'; jt++)
					{
						if (word[it] == illegalword[jt])
						{
							...
							ROS_WARN("Annotation name [%s] contains an illegal character '%c'. Please name the annotation only with a combination of letters, numbers and _ (underscore). Do not use any character from the list of illegal characters %s. Renaming the annotation to [%s]!", word, word[it], illegalword, annt.name.c_str());
							validname = false;
							unchanged = false;
							break;
							//							return 0;
				...
				for (uint k = 0; k < annotations.annotations.size(); k++)
				{
					...
					{
						annt.name = "location_" + std::to_string(annotations.annotations.size());
						ROS_WARN("Two annotations have the same name [%s]! Names must be unique! Renaming to [%s]!", annotations.annotations[k].name.c_str(), annt.name.c_str());
						validname = false;
						unchanged = false;
						break;
					}
...
int annotationsToGraph()
{
	...
	while (k < annotations.annotations.size())
	{
		...
		if (is >= 0 && js >= 0 && is < sizex && js < sizey)
		{
			...
			if (gmap[is][js].occupancy == 0)
			{
				...
				for (uint l = k + 1; l < annotations.annotations.size(); l++)
				{
					...
					if ((ig == is) && (jg == js))
					{
						ROS_WARN("Annotations [%s] and [%s] fall into the same topology grid cell with map coordinates (%d, %d). Deleting the annotation [%s]... Change the coordinates or decrease the cell size.", annotations.annotations[l].name.c_str(), annotations.annotations[k].name.c_str(), is, js, annotations.annotations[l].name.c_str());
					...
					{
						ROS_WARN("The annotation [%s] has vertex coordinates at (%f, %f) which is less than the cell_size = %f m distanced from the annotation [%s] vertex coordinates (%f, %f). Deleting the annotation [%s]... Change the coordinates or decrease the cell size.", annotations.annotations[l].name.c_str(), midx, midy, cellsize, annotations.annotations[k].name.c_str(), tempx, tempy, annotations.annotations[l].name.c_str());
						annotations.annotations.erase(annotations.annotations.begin() + l);
						unchanged = false;
						break;
					}
			...
			else
			{
				...
				ROS_WARN("Annotation [%s] is occupied with its vertex coordinates at (%f, %f) for the cell_size = %f m!!! Deleting occupied annotation... Change the coordinates or exit and edit manually annotations.ini file.", annotations.annotations[k].name.c_str(), tempx, tempy, cellsize);
		...
		else
		{
			...
			ROS_WARN("Annotation [%s] is out of map borders with its vertex map coordinates (%d, %d)!!! Deleting invalid annotation... Change the coordinates or or exit and edit manually annotations.ini file.", annotations.annotations[k].name.c_str(), is, js);
			annotations.annotations.erase(annotations.annotations.begin() + k);
			unchanged = false;
			continue;
		}
		...
		for (int i = 0; i < sizex; i++)
		{
			for (int j = 0; j < sizey; j++)
			{
				if (gmap[i][j].occupancy == 0)
				{
...
#if USE_SPANNING_TREE
					if (gmap[i][j].visited == 1)
					{ //only part of the graph that is connected to annotations
						gmap[i][j].visited = 0;
#endif
						vertex.x = gmap[i][j].x;
						vertex.y = gmap[i][j].y;
					...
					{ //the part of the graph disconnected from annotations is occupied
...
void newAnnotationCallback(const geometry_msgs::PoseStamped::ConstPtr &goal)
{
	...
	if (annotationsToGraph())
	{
		...
		for (uint k = 0; k < annotations.annotations.size(); k++)
		{
			...
			out << "distance = " << annotations.annotations[k].distance << std::endl
				<< std::endl;
		}
		out.close();

		std::cout << "New selected annotation:" << std::endl
				  << std::endl;
